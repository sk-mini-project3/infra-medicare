...
이 문서는 AWS 계정을 아직 받지 못한 상태에서 시작해, 계정을 받은 뒤 무엇부터 해야 하는지, GitHub Actions로 이미지를 ECR에 올리고, Terraform으로 EKS와 RDS를 만들고, Argo CD로 배포하기까지의 전 과정을 한 파일로 정리한 실행 매뉴얼입니다.
...
5. Terraform 1차 실행으로 ECR/VPC/RDS 생성, 2차 실행으로 EKS 생성
...
- `rds_address` = RDS PostgreSQL 호스트명 (예: `medical-service-db.c1234.ap-northeast-2.rds.amazonaws.com`)
- `rds_endpoint` = RDS 호스트:포트 (예: `medical-service-db.c1234.ap-northeast-2.rds.amazonaws.com:5432`)
- `db_name` = 데이터베이스 이름
...
- [ ] Terraform 1차 실행으로 ECR/VPC/RDS 생성 완료
