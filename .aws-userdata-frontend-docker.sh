...
IMG=ghcr.io/sk-mini-project3/frontend-medicare:latest
...
if [[ -n "${SECRET_JSON:-}" && "${SECRET_JSON}" != "None" ]]; then
  ...
  echo "No Secrets Manager value for medicare/ghcr-read (anonymous pull; private GHCR will fail)"
fi

docker pull "$IMG"
docker stop medicare-frontend 2>/dev/null || true
docker rm medicare-frontend 2>/dev/null || true
